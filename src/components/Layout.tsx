...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navigation = [
    ...
    { name: 'Emitir NFe', href: '/nfe/emitir', icon: DocumentTextIcon },
    { name: 'Histórico', href: '/nfe/historico', icon: ClockIcon },
      ...
      <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
        ...
        <div className="fixed inset-y-0 left-0 flex w-64 flex-col bg-white shadow-xl">
          <div className="flex h-16 items-center justify-between px-4">
            <h1 className="text-xl font-bold text-gray-900">NFe Brandão</h1>
            <button
      ...
      <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
        <div className="flex flex-col flex-grow bg-white border-r border-gray-200">
          <div className="flex h-16 items-center px-4">
            <h1 className="text-xl font-bold text-gray-900">NFe Brandão Contador</h1>
          </div>
          <nav className="flex-1 space-y-1 px-2 py-4">
            {navigation.map((item) => {
              const isActive = location.pathname === item.href
