...
import { FileText, Truck, Users, Calculator, AlertTriangle, CheckCircle, Plus, Trash2 } from 'lucide-react';
import { DadosMDFe, DocumentoVinculado, calcularMDFe, validarPrazosMDFe, validarVinculoCteMdfe } from '../../utils/calculosMDFe';
...
export function FormularioMDFe({ onSubmit, onPreview }: FormularioMDFeProps) {
  ...
  const [novoDocumento, setNovoDocumento] = useState<Partial<DocumentoVinculado>>({
    tipo: 'CTe',
    chave: '',
    valor: 0,
    peso: 0,
    ...
    <div className="space-y-6">
          ...
          {[1, 2, 3, 4].map((etapa) => (
            <div key={etapa} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                etapa <= etapaAtual ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-600'
              }`}>
                {etapa}
              </div>
              <span className={`text-sm ${etapa <= etapaAtual ? 'text-green-600' : 'text-gray-500'}`}>
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Select
              ...
              options={[
                { value: 'eta', label: 'ETA - Empresa de Transporte de Cargas' },
                { value: 'tac', label: 'TAC - Transportador Autônomo de Cargas' },
                { value: 'etc', label: 'ETC - Empresa de Transporte de Cargas' },
              ]}
              required
            />
      ...
      {etapaAtual === 2 && (
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {dados.modal === 'rodoviario' && (
              <Input
                ...
                onChange={(e) => handleInputChange('placaCarreta', e.target.value.toUpperCase())}
                placeholder="DEF5678"
              />
            )}
            ...
            <Input
              ...
              onChange={(e) => handleInputChange('capacidadeKg', parseFloat(e.target.value) || 0)}
              placeholder="0"
            />
            ...
            {dados.modal === 'aquaviario' && (
              <Input
                ...
                onChange={(e) => handleInputChange('capacidadeM3', parseFloat(e.target.value) || 0)}
                placeholder="0,000"
              />
            )}
            ...
            <Input
              ...
              onChange={(e) => handleInputChange('cpfCondutor', e.target.value.replace(/\D/g, ''))}
            ...
            <Input
              ...
              onChange={(e) => handleInputChange('nomeCondutor', e.target.value)}
          ...
          {/* Dados do seguro */}
          <div className="mt-6">
            <h4 className="text-md font-medium text-gray-700 mb-3">Dados do Seguro (Opcional)</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <Input
              ...
              <Input
                ...
                onChange={(e) => handleInputChange('apoliceSeguro', e.target.value)}
              ...
              <Input
                ...
                onChange={(e) => handleInputChange('seguradoraRespCivil', e.target.value)}
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
              <h4 className="font-medium text-gray-700 mb-3">Adicionar Documento</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                <Select
                  label="Tipo"
                  value={novoDocumento.tipo || 'CTe'}
                  onChange={(value) => setNovoDocumento(prev => ({ ...prev, tipo: value as 'CTe' | 'NFe' }))}
                  options={[
                    { value: 'CTe', label: 'CTe' },
                    { value: 'NFe', label: 'NFe' },
                  ]}
                ...
                <Input
                  ...
                  onChange={(e) => setNovoDocumento(prev => ({ ...prev, chave: e.target.value }))}
                ...
                <Input
                  ...
                  onChange={(e) => setNovoDocumento(prev => ({ ...prev, valor: parseFloat(e.target.value) || 0 }))}
                  placeholder="0,00"
                />
                ...
                <Input
                  ...
                  onChange={(e) => setNovoDocumento(prev => ({ ...prev, peso: parseFloat(e.target.value) || 0 }))}
                  placeholder="0,000"
                />
            ...
            {documentos.length > 0 && (
              <div className="space-y-2">
                <h4 className="font-medium text-gray-700">Documentos Adicionados ({documentos.length})</h4>
                <div className="space-y-2">
                  {documentos.map((doc, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-white border rounded-lg">
                      <div className="flex items-center gap-4">
                        <Badge variant={doc.tipo === 'CTe' ? 'default' : 'secondary'}>
                          {doc.tipo}
                        </Badge>
                        <span className="text-sm font-mono">{doc.chave}</span>
                        <span className="text-sm text-gray-600">R$ {doc.valor.toFixed(2)}</span>
                        <span className="text-sm text-gray-600">{doc.peso.toFixed(3)}kg</span>
                      </div>
                      <button
            ...
            {documentos.length > 0 && (
              <div className="mt-6 p-4 bg-green-50 rounded-lg">
                <h4 className="font-medium text-green-900 mb-2">Resumo dos Totais</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    <span className="text-green-700">CTe:</span>
                    <span className="font-medium ml-2">{dados.quantidadeCTe}</span>
                  </div>
                  <div>
                    <span className="text-green-700">NFe:</span>
                    <span className="font-medium ml-2">{dados.quantidadeNFe}</span>
                  </div>
                  <div>
                    <span className="text-green-700">Peso Total:</span>
                    <span className="font-medium ml-2">{dados.pesoTotal.toFixed(3)}kg</span>
                  </div>
                  <div>
                    <span className="text-green-700">Valor Total:</span>
                    <span className="font-bold ml-2">R$ {dados.valorTotal.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            )}
      ...
      {etapaAtual === 4 && calculoResult && (
        <Card className="p-6">
          ...
          <div className="mb-6 p-4 bg-green-50 rounded-lg">
            <h4 className="font-medium text-green-900 mb-2">Resumo Final do MDFe</h4>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
              ...
              <div>
                <span className="text-green-700">Peso Total:</span>
                <span className="font-medium ml-2">{calculoResult.pesoTotal.toFixed(3)}kg</span>
              </div>
              <div>
                <span className="text-green-700">Valor Total:</span>
              ...
              <div>
                ...
                <span className={`font-medium ml-2 ${calculoResult.validacoes.valido ? 'text-green-600' : 'text-red-600'}`}>
