  ...
  const authAPI = {
    login: async (email: string, password: string) => {
      ...
      try {
        ...
        throw new Error(
          `Erro ao processar resposta do servidor: ${responseText}`,
        );
      }
      ...
      const rawPerms: string[] = Array.isArray(data.usuario.permissoes)
        ? data.usuario.permissoes
        : [];
      const isAdmin = rawPerms.some(
        (p: string) => p === "admin" || p === "admin_total",
      );
      const normalizedPerms = isAdmin
        ? rawPerms
        : Array.from(new Set([...rawPerms, "configuracoes_ver"]));
