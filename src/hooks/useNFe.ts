...
export const useNFe = (): UseNFeReturn => {
      ...
      try {
        ...
        setPagination({
          ...
          totalPages: Math.ceil(
            (data.total || lista.length) / (data.limite || params.limite),
          ),
        });
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao carregar NFes";
  ...
  const loadNFeByChave = useCallback(
    async (chave: string): Promise<NFe | null> => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        const nfe: NFe = {
          ...
          destinatario: {
            nome:
              data.destinatario?.nome || data.destinatario || "Destinatário",
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao consultar NFe";
        setError(errorMessage);
        showToast(errorMessage, "error");
        return null;
      } finally {
  ...
  const emitirNFe = useCallback(
    async (nfeData: any): Promise<boolean> => {
      setIsLoading(true);
      setError(null);
  ...
  const cancelarNFe = useCallback(
    async (chave: string, justificativa: string): Promise<boolean> => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao cancelar NFe";
        setError(errorMessage);
        showToast(errorMessage, "error");
        return false;
      } finally {
  ...
  const inutilizarNFe = useCallback(
    async (
      serie: number,
      numeroInicial: number,
      numeroFinal: number,
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao inutilizar numeração";
        setError(errorMessage);
        showToast(errorMessage, "error");
        return false;
      } finally {
  ...
  const downloadXML = useCallback(
    async (chave: string): Promise<void> => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Erro ao baixar XML";
        showToast(errorMessage, "error");
      }
    },
    [showToast],
  ...
  const enviarEmail = useCallback(
    async (chave: string, email: string): Promise<boolean> => {
      setIsLoading(true);
      setError(null);
