...
const Relatorios: React.FC = () => {
  ...
  const [tipoRelatorio, setTipoRelatorio] = useState<"vendas" | "impostos">(
    "vendas",
  );
  ...
  const [relatorioImpostos, setRelatorioImpostos] =
    useState<RelatorioImpostos | null>(null);
  ...
  const exportarRelatorio = async (formato: "pdf" | "excel") => {
    try {
      if (tipoRelatorio === "vendas" && !relatorioVendas) {
        showToast(
          "Nenhum dado de vendas para exportar no período selecionado.",
          "info",
        );
        return;
      }
      if (tipoRelatorio === "impostos" && !relatorioImpostos) {
        showToast(
          "Nenhum dado de impostos para exportar no período selecionado.",
          "info",
        );
        return;
      }
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <FormGroup label="Tipo de Relatório">
                <Select
                  value={tipoRelatorio}
                  onChange={(e) =>
                    setTipoRelatorio(e.target.value as "vendas" | "impostos")
                  }
                >
            ...
            <CardBody>
              <div className="py-12 text-center">
                ...
                <p className="text-gray-500 text-sm mt-1">
                  Emita NFes para visualizar KPIs e gráficos aqui.
                </p>
              </div>
            </CardBody>
          </Card>
              ...
              <CardBody>
                <div className="space-y-4">
                  {[
                    {
                      nome: "ICMS",
                      valor: relatorioImpostos.icms,
                      cor: "bg-red-500",
                    },
                    {
                      nome: "COFINS",
                      valor: relatorioImpostos.cofins,
                      cor: "bg-purple-500",
                    },
                    {
                      nome: "IPI",
                      valor: relatorioImpostos.ipi,
                      cor: "bg-orange-500",
                    },
                    {
                      nome: "PIS",
                      valor: relatorioImpostos.pis,
                      cor: "bg-blue-500",
                    },
                  ].map((imposto) => {
                    const percentual =
                      (imposto.valor / relatorioImpostos.total) * 100;
                    return (
                      <div
                        key={imposto.nome}
                        className="flex items-center justify-between p-3 bg-gray-50 rounded-lg"
                      >
                        <div className="flex items-center space-x-3">
                          <div
                            className={`w-4 h-4 rounded ${imposto.cor}`}
                          ></div>
                          <span className="font-medium text-gray-900">
                            {imposto.nome}
                          </span>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {formatarMoeda(relatorioImpostos.total)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Maior Imposto:</span>
                        <span className="font-medium">
                          ICMS ({formatarMoeda(relatorioImpostos.icms)})
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      ...
                      <p>
                        • ICMS representa{" "}
                        {(
                          (relatorioImpostos.icms / relatorioImpostos.total) *
                          100
                        ).toFixed(1)}
                        % do total
                      </p>
        ...
        {tipoRelatorio === "impostos" && !relatorioImpostos && (
          <Card>
            <CardBody>
              <div className="py-12 text-center">
                <p className="text-gray-700 font-medium">
                  Nenhum dado de impostos para o período selecionado.
                </p>
                <p className="text-gray-500 text-sm mt-1">
                  Emita e atualize NFes para visualizar os resumos fiscais.
                </p>
              </div>
            </CardBody>
          </Card>
