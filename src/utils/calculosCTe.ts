...
export interface DadosCTe {
  tipoServico: 'normal' | 'subcontratacao' | 'redespacho' | 'intermediacao' | 'multimodal';
  modal: 'rodoviario' | 'aereo' | 'aquaviario' | 'ferroviario' | 'dutoviario';
  tipoFrete: 'cif' | 'fob' | 'terceiros' | 'proprio_remetente' | 'proprio_destinatario';
...
export function calcularCTe(dados: DadosCTe): ResultadoCalculoCTe {
  ...
  if (dados.baseCalculoICMS && dados.aliquotaICMS) {
    ...
    observacoes.push(`ICMS: Base R$ ${dados.baseCalculoICMS.toFixed(2)} x ${dados.aliquotaICMS}% = R$ ${valorICMS.toFixed(2)}`);
  }
  ...
  if (dados.baseCalculoIS && dados.aliquotaIS) {
    ...
    observacoes.push(`IS 2026: Base R$ ${dados.baseCalculoIS.toFixed(2)} x ${dados.aliquotaIS}% = R$ ${valorIS.toFixed(2)}`);
  }
  ...
  observacoes.push('CTe emitido conforme Lei 12.741/2012 - Valor aproximado dos tributos');
  observacoes.push('Documento auxiliar do Conhecimento de Transporte Eletrônico');
  
  if (dados.tipoServico === 'subcontratacao') {
    observacoes.push('Serviço de transporte subcontratado conforme art. 31 da Lei 11.442/2007');
  }
  
  // Preparação 2026
  if (valorIBS > 0 || valorCBS > 0 || valorIS > 0) {
    observacoes.push('Valores IBS/CBS/IS calculados conforme Reforma Tributária 2026 (Lei Complementar nº 212/2024)');
  }
  
  const valorTotal = dados.valorTotalServico + valorICMS + valorIBS + valorCBS + valorIS;
 ...
 */
export function validarPrazosCTe(dataEmissao: Date, dataVencimento?: Date): {
  valido: boolean;
  ...
  const horasRestantes = Math.max(0, (limite168h.getTime() - agora.getTime()) / (60 * 60 * 1000));
...
export function gerarObservacoesCTe(dados: DadosCTe): string[] {
  ...
  switch (dados.tipoServico) {
    ...
    case 'subcontratacao':
      ...
      observacoes.push('Responsabilidade solidária conforme art. 31 da Lei 11.442/2007');
      break;
    case 'redespacho':
