 ...
 */
export function calcularMDFe(
  dados: DadosMDFe,
  documentos: DocumentoVinculado[],
): ResultadoCalculoMDFe {
  ...
  const valorDocumentos = documentos.reduce(
    (total, doc) => total + doc.valor,
    0,
  );
  ...
  if (Math.abs(valorDocumentos - dados.valorTotal) > 0.01) {
    erros.push(
      `Divergência no valor total: MDFe R$ ${dados.valorTotal.toFixed(2)} vs Documentos R$ ${valorDocumentos.toFixed(2)}`,
    );
  }

  if (Math.abs(pesoDocumentos - dados.pesoTotal) > 0.01) {
    erros.push(
      `Divergência no peso total: MDFe ${dados.pesoTotal.toFixed(2)}kg vs Documentos ${pesoDocumentos.toFixed(2)}kg`,
    );
  }
  ...
  if (dados.capacidadeKg && dados.pesoTotal > dados.capacidadeKg) {
    avisos.push(
      `Peso total (${dados.pesoTotal.toFixed(2)}kg) excede capacidade do veículo (${dados.capacidadeKg.toFixed(2)}kg)`,
    );
  }
  ...
  observacoes.push(
    "MDFe emitido conforme Portaria 1.100/2014 do Ministério da Fazenda",
  );
  ...
  if (dados.valorSeguro && dados.apoliceSeguro) {
    observacoes.push(
      `Seguro: R$ ${dados.valorSeguro.toFixed(2)} - Apólice: ${dados.apoliceSeguro}`,
    );
  }
  ...
  if (valorIBS > 0 || valorCBS > 0) {
    observacoes.push(
      "Valores IBS/CBS calculados conforme Reforma Tributária 2026 (Lei Complementar nº 212/2024)",
    );
  }
 ...
 */
export function validarPrazosMDFe(
  dataEmissao: Date,
  ...
  if (dataEncerramento && dataEncerramento > limite24h) {
    return {
      ...
      horasRestantes: 0,
      mensagem:
        "MDFe deve ser encerrado em até 24 horas quando vinculado a CTe",
    };
  }

  const horasRestantes = Math.max(
    0,
    (limite24h.getTime() - agora.getTime()) / (60 * 60 * 1000),
  );
...
export function validarVinculoCteMdfe(
  ...
  const diferencaDias =
    (dataMDFe.getTime() - dataCtE.getTime()) / (24 * 60 * 60 * 1000);
  if (diferencaDias > 7) {
    return {
      valido: false,
      mensagem:
        "CTe não pode ser vinculado a MDFe com mais de 7 dias de diferença",
    };
  }
...
export function gerarObservacoesMDFe(dados: DadosMDFe): string[] {
  ...
  switch (dados.modal) {
    case "rodoviario":
      ...
      if (dados.placaCarreta) {
        observacoes.push(
          `Veículo: ${dados.placaVeiculo} + Carreta: ${dados.placaCarreta}`,
        );
      } else {
  ...
  if (
    Array.isArray(dados.municipiosPercurso) &&
    dados.municipiosPercurso.length > 0
  ) {
  ...
  observacoes.push(
    `Documentos vinculados: ${dados.quantidadeCTe} CTe + ${dados.quantidadeNFe} NFe`,
  );
 ...
 */
export function calcularDistanciaUF(
  ufOrigem: string,
  ufDestino: string,
): number {
