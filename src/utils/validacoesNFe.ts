...
export function validarValor(valor: number, campo: string): ValidacaoResult {
  ...
  if (decimais && decimais.length > 2) {
    return { valido: false, erro: `${campo} deve ter no máximo 2 casas decimais` }
  }
...
export function validarQuantidade(quantidade: number): ValidacaoResult {
  ...
  if (decimais && decimais.length > 4) {
    return { valido: false, erro: 'Quantidade deve ter no máximo 4 casas decimais' }
  }
  
  // Validar quantidade máxima
  if (quantidade > 999999999.9999) {
    return { valido: false, erro: 'Quantidade excede o valor máximo permitido' }
  }
...
export function validarGTIN(gtin: string): ValidacaoResult {
  ...
  if (checkDigit !== lastDigit) {
    return { valido: false, erro: 'GTIN inválido - dígito verificador incorreto' }
  }
...
export function validarCampos2026(item: any): ValidacaoResult[] {
  ...
  if (item.ibs !== undefined || item.cbs !== undefined || item.is !== undefined) {
    validacoes.push({
      valido: true,
      aviso: 'Campos IBS/CBS/IS detectados - Sistema preparado para Reforma Tributária 2026'
    })
  }
